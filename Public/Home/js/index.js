...
$(function() {
	...
	$(".introduction_ty5 li:first").css('margin-left', '50px');
	...
	$(".xyfx li").hide();
	$(".xyfx li:first").show();
	$(".cont_ty29 li").hover(function(){
		var tindex = $(this).index();
		$(".xyfx li").eq(tindex).show().siblings().hide();
	});
	function ckdx(){
	...
	var int;
	var img1 = "<img src='" + __IMG__ + "/jkd_20.png' class='tcircle' />";
	var img2 = "<img src='" + __IMG__ + "/jkd_21.png' class='tcircle' />";
	...
	$(".kvideo_ty13 li").hover(function() {
		var bh = parseInt($(this).find('b').height())+20;
		$(this).addClass('kvideo_ty25').siblings('li').removeClass('kvideo_ty25');
		$(this).children('.kvideo_ty21').stop(true).animate({height: '+=' + bh + 'px','padding-top':'10px',width:'270px','margin-left': '-10px'}, 800).addClass('kvideo_ty24');
	...
	$('.qds span:last').remove();
	$(".cont_ty29").hover(function(){
		$(".xyfx").stop(true).toggle();
	});
	function pg(i){
		var pimg = $(i).children('img');
		pimg.each(function() {
	...
	pg(".cont_ty32 li");
	var lnlen = $(".leftNav li").size();
    $(".leftNav li").each(function() {
        var $index = $(this).index()+1;
        if($index < 10){
            $index = "0" + $index;
        }
        $(this).children('a').prepend("<i>" + $index + "</i>");
    });
});
